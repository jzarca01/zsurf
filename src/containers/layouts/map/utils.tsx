...
function haversineKm(fromLocation, toLocation) {
  ...
  const dLat = deg2rad(fromLocation.latitude - toLocation.latitude);
  const dLng = deg2rad(fromLocation.longitude - toLocation.longitude);

  const a =
    Math.sin(dLat / 2) * Math.sin(dLat / 2) +
    Math.sin(dLng / 2) *
      Math.sin(dLng / 2) *
      Math.cos(deg2rad(fromLocation.latitude)) *
      Math.cos(deg2rad(toLocation.latitude));
 ...
 *
 * @param  {google.maps.latitudeLngBounds} bounds   The google maps bounds object
 *
 * @return {number}
 */
function calculateRadiusFromMapBounds(bounds) {
  ...
  const pointNELatLng = {
    lat: pointNE.latitude(),
    lng: pointNE.longitude()
  };
  const pointSW = bounds.getSouthWest();
  const pointSWLatLng = {
    lat: pointSW.latitude(),
    lng: pointSW.longitude()
  };

  // calculate the diagonal
  const diagonalDistanceKm = haversineKm(pointNELatLng, pointSWLatLng);
...
function isLatAndLngInRectBoundary(point, boundary) {
  ...
  const eastBound = point.longitude < boundary.NE.longitude;
  const westBound = point.longitude > boundary.SW.longitude;

  var inLng;

  if (boundary.NE.longitude < boundary.SW.longitude) {
  ...
  var inLat =
    point.latitude > boundary.SW.latitude &&
    point.latitude < boundary.NE.latitude;
